...
Forwards LightRAG and RAG-Anything logs to EduFlow's unified logging system.
...
class LightRAGLogForwarder(logging.Handler):
    """
    Handler that forwards LightRAG logger messages to EduFlow logger.
    """

    def __init__(self, ai_tutor_logger, add_prefix: bool = True):
        """
        Args:
            ai_tutor_logger: EduFlow Logger instance
    ...
    def emit(self, record: logging.LogRecord):
        """
        Forward log record to EduFlow logger with proper level mapping.
        """
        try:
            message = record.getMessage()

            # Map LightRAG log levels to EduFlow logger methods
