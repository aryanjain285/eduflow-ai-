...
This directory contains all configuration files for the EduFlow system. Configuration files are organized by module and utilize YAML format.
...
1. **Environment Variables** (`.env` or `EduFlow.env`)
...
Required environment variables (in `.env` or `EduFlow.env`):
...
2. **Model configuration**: LLM model names should only be set via environment variables (`.env` or `EduFlow.env`), not in config files.
